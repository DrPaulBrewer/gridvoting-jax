...
### Added - Partition Class for Symmetry Metadata

- **`Partition` Class**: New data class wrapping partition arrays with optimization metadata
  - **Location**: `src/gridvoting_jax/geometry.py:32-81`
  - **Attributes**:
    - `inverse_indices`: JAX array mapping each state to its group (0 to k-1)
    - `metadata`: `PartitionMetadata` object with optimization hints
    - `k`: Number of groups in partition
  - **Methods**:
    - `__len__()`: Returns number of states for `len(partition)` calls
    - `max()`: Returns maximum value for backward compatibility
  - **JAX Integration**: Registered as JAX pytree for JIT compatibility

- **`PartitionMetadata` Dataclass**: Optimization hints for lumping operations
  - **Attributes**:
    ...
    - `source`: Descriptive string identifying partition origin

### Changed - Partition-Returning Functions

- **`Grid.partition_from_symmetry()`**: Now returns `Partition` objects instead of raw arrays
  - **Single symmetries**: Delegates to `parts_from_linear_discriminator` with symmetry type
  - **Multiple symmetries**: Creates descriptive source string (e.g., `"reflect_x_reflect_y"`)
  - **Empty symmetries**: Returns identity partition with `lazy_lumpable=False`
  - **Rotation symmetries**: Source includes center point (e.g., `"rotate_0_0_120deg"`)

- **`Grid.parts_from_linear_discriminator()`**: Now returns `Partition` objects
  - **New parameter**: `symmetry_type` for metadata source string
  - **Metadata**: `primary_group_size=2` with singleton exceptions for reflection axis points
  - **Backward compatible**: Added `max()` method to Partition class

- **`PolarGrid.partition_from_rotation()`**: Already returns `Partition` objects (v0.44.0)
  - **Consistent API**: All partition-generating methods now return `Partition` objects

- **`SpatialVotingModel.get_spatial_symmetry_partition()`**: Updated docstring
  - **Return type**: Now documented as returning `Partition` objects
  - **PolarGrid warning**: Added `NotImplementedError` documentation for PolarGrid usage
  - **Clarification**: Only works with Cartesian `Grid`, not `PolarGrid`

### Enhanced - Lumping Functions

- **`lump()`, `unlump()`, `is_lumpable()`**: Already support `Partition` objects (v0.44.0)
  - **Automatic extraction**: Functions extract `.inverse_indices` when passed `Partition` objects
  - **Backward compatible**: Still accept raw JAX arrays

- **`count_mismatches()`**: Updated to handle `Partition` objects (v0.44.0)
  - **Flexible input**: Accepts both `Partition` objects and raw arrays

### Technical Details

**Metadata Strategy**:
- **Reflection symmetries**: `primary_group_size=2`, exceptions for singleton groups on reflection axis
- **Rotation symmetries**: `primary_group_size=None` (variable due to tolerance-based matching)
- **Multiple symmetries**: `primary_group_size=None`, descriptive concatenated source string
- **Identity partition**: `primary_group_size=1`, `lazy_lumpable=False`

**Source String Formats**:
- Single reflection: `"reflect_x"`, `"reflect_y=1.5"`
- Single rotation: `"rotate_0_0_120deg"` (includes center point)
- Multiple symmetries: `"reflect_x_reflect_y"`, `"reflect_x_rotate_0_0_120deg"`
- Identity: `"identity"`

**Test Updates**:
- Updated 9 spatial symmetry tests in `test_geometry_symmetry.py` to use `.inverse_indices`
- All 214 tests passing

**Out of Scope**:
- `partition_from_permutation_symmetry()`: Still returns raw arrays (separate task)
- `PolarGrid.partition_from_symmetry()`: Still raises `NotImplementedError` (deferred)
