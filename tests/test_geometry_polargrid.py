...
def test_polargrid_ring_sums():
    ...
    assert jnp.allclose(pg.ring_sums(z=pg.x), jnp.zeros(pg.n_rvals), atol=20.0*jnp.finfo(jnp.float32).eps)
    assert jnp.allclose(pg.ring_sums(z=pg.y), jnp.zeros(pg.n_rvals), atol=20.0*jnp.finfo(jnp.float32).eps)
...
def test_polargrid_theta_sums():
    ...
    assert jnp.allclose(pg.theta_sums(z=pg.x), 55.0*pg.unit_vectors[:,0])
    assert jnp.allclose(pg.theta_sums(z=pg.y), 55.0*pg.unit_vectors[:,1])
